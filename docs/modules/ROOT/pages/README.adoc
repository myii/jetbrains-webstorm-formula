...
https://travis-ci.com/saltstack-formulas/webstorm-formula[image:https://travis-ci.com/saltstack-formulas/webstorm-formula.svg?branch=master[Travis CI Build Status]]
...
This installs Webstorm package, manages Webstorm configuration file and
...
on `webstorm.install` via include list.
...
this state will undo everything performed in the `webstorm` meta-state
...
Creates the docker instance and runs the `webstorm` main state, ready
for testing.
